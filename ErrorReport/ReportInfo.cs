...
namespace ErrorReport
{
    class ReportInfo
    {
        ...
        public ReportInfo(List<JobHistory> jobHistory, int numOfTrunk)
        {
            ...
            int elapsedTime = (jobHistory[jobHistory.Count - 1].EndTime - jobHistory[0].EndTime).Days;
            DateTime tempDate = jobHistory[0].EndTime;
            int interval = elapsedTime / numOfTrunk;
            ...
            {
                if (job.EndTime > tempDate.AddDays(numOfTrunk))
                {
                    currentTrunk++;
                    tempDate = job.EndTime;
                }
